...
const correctAnswers = ['b', 'a', 'b', 'c', 'a', 'a', 'b', 'b', 'b', 'b', 'c', 'c', 'c', 'a', 'b'];
...
form.addEventListener('submit', event => {
    ...
    if (userPoints >= 0 && userPoints <= 8) {
        knowledgeLevel = 'Excel Avançado I (Iniciante)';
        recommendationText = 'Recomendação: Curso Excel Avançado I (Iniciante)';
    } else if (userPoints >= 9 && userPoints <= 13) {
        knowledgeLevel = 'Excel Avançado I (Consolidar base)';
        recommendationText = 'Recomendação: Curso Excel Avançado I (Consolidar base)';
    } else if (userPoints >= 14 && userPoints <= 15) {
        knowledgeLevel = 'Excel Avançado II';
        recommendationText = 'Recomendação: Pular para o curso Excel Avançado II';
    } else {
    ...
    switch (knowledgeLevel) {
        case 'Excel Avançado I (Iniciante)':
            recommendationClass = 'level-excel-avancado-i-iniciante';
            break;
        case 'Excel Avançado I (Consolidar base)':
            recommendationClass = 'level-excel-avancado-i-consolidar';
            break;
        case 'Excel Avançado II':
            recommendationClass = 'level-excel-avancado-ii';
            break;
        default:
