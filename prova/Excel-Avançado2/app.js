...
const correctAnswers = ['c', 'a', 'c', 'b', 'b', 'c', 'a', 'c', 'c', 'c', 'd', 'a', 'b', 'c', 'c'];
...
form.addEventListener('submit', event => {
    ...
    if (userPoints >= 0 && userPoints <= 8) {
        knowledgeLevel = 'Excel Avançado II (Iniciante)';
        recommendationText = 'Recomendação: Curso Excel Avançado II (Iniciante)';
    } else if (userPoints >= 9 && userPoints <= 13) {
        knowledgeLevel = 'Excel Avançado II (Consolidar base)';
        recommendationText = 'Recomendação: Curso Excel Avançado II (Consolidar base)';
    } else if (userPoints >= 14 && userPoints <= 15) {
        knowledgeLevel = 'Profissional em Excel';
        recommendationText = 'Parabéns! Você já é um profissional em Excel.';
    } else {
    ...
    switch (knowledgeLevel) {
        case 'Excel Avançado II (Iniciante)':
            recommendationClass = 'level-excel-avancado-ii-iniciante';
            break;
        case 'Excel Avançado II (Consolidar base)':
            recommendationClass = 'level-excel-avancado-ii-consolidar';
            break;
        case 'Profissional em Excel':
            recommendationClass = 'level-profissional-excel';
            break;
        default:
